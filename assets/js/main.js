...
function initClientsCarousel() {
  ...
  // Contar cuántos elementos hijo hay
  const childrenCount = track.children.length;
  
  // Si hay menos de 12 elementos, duplicamos hasta tener suficiente
  if (childrenCount < 12) {
    const originalHTML = track.innerHTML;
    // Duplicamos 2 veces para asegurar ancho suficiente
    track.innerHTML = originalHTML + originalHTML + originalHTML;
  }
  
  // Forzar que la animación se aplique reiniciando el CSS
  ...
  console.log('Carrusel iniciado con', track.children.length, 'elementos');
}
